...
                .query('SELECT COUNT(*) as count FROM [Likes] WHERE Comment_id = @comment_id');
...
                    .input('comment_id', sql.BigInt, commentIdNum)
                    .query('INSERT INTO [Likes] (User_id, Post_id, Comment_id) VALUES (@user_id, @post_id, @comment_id)');
            } catch (insError1) {
...
        const pool = await connection();
        
        // 1. Kiểm tra chủ sở hữu (Vẫn cần thiết)
        const postResult = await pool.request()
            .input('post_id', sql.BigInt, postIdNum)
...
        // 3. Gọi Stored Procedure để xóa
        await pool.request()
            .input('PostID', sql.BigInt, postIdNum) // Tên tham số khớp với SP
            .execute('sp_DeletePost'); // Gọi SP đã sửa
...
        res.status(500).json({ 
            error: 'Lỗi server', 
            detail: error.originalError?.message || error.message 
        });
    }
};
